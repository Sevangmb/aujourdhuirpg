...
// AI Input Preparation - This is called ONLY for the prologue.
// The main game loop uses the AIContextPreparer.
export function prepareAIInput(gameState: GameState, playerChoice: StoryChoice | { text: string }): any | null {
  if (!gameState.player) {
  ...
  // CORRECT: Create nested skills object for AI
  const nestedSkills = (Object.keys(player.skills) as Array<keyof AdvancedSkillSystem>).reduce((acc, category) => {
    ...
    for (const skillName in skillsInCategory) {
        subSkills[skillName] = (skillsInCategory as any)[skillName].level;
    }
    (acc as any)[category] = subSkills;
    return acc;
  ...
  return {
    ...
    gameEvents: JSON.stringify([], null, 2),
    cascadeResult: JSON.stringify({}, null, 2),
