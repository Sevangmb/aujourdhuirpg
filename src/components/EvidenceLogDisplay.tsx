...
import { Search, FileText as DocumentIcon, Lightbulb, MessageSquare } from 'lucide-react'; // Lightbulb for Clues, MessageSquare for notes
...
const EvidenceLogDisplay: React.FC<EvidenceLogDisplayProps> = ({ player }) => {
  ...
  return (
    <div className="p-1 h-full flex flex-col flex-grow">
      <Tabs defaultValue="clues" className="w-full flex-grow flex flex-col">
        ...
        <ScrollArea className="flex-grow pr-1 min-h-0">
          <TabsContent value="clues" className="mt-0 pt-1">
            {clues.length > 0 ? (
          ...
          <TabsContent value="documents" className="mt-0 pt-1">
            {documents.length > 0 ? (
          ...
          <TabsContent value="summary" className="mt-0 pt-1">
            <Card>
              <CardHeader>
