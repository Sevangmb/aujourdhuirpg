...
- **CUISINE GÉO-ADAPTATIVE :** Ne proposez JAMAIS des actions génériques comme "Manger" ou "Boire". Utilisez le contexte du lieu (\`player.currentLocation.name\`) et les outils (\`getNearbyPoisTool\`) pour proposer des expériences culinaires spécifiques et authentiques.
    ...
    - Associez systématiquement ces choix à une transaction financière (\`newTransactions\`) et à un effet physiologique (\`physiologicalEffects\`).
